...
mat2D<int> * Embed(mat2D<int> *cover, cost_model_config * config, float &alpha_out, float &coding_loss_out, unsigned int &stc_trials_used, float &distortion, std::string payload_data);
...
int main(int argc, char** argv)
{
	try { 
        ...
        desc.add_options()
            ...
            ("payload,a",				po::value<float>(&payload_rate),								"payload to embed in bits per non-zero AC DCT coefficient")
			("payload_path,m",			po::value<std::string>(&payload_path),							"payload data to embed")
		...
		if (payload_rate<=0){ std::cout << "'payload rate' must be larger than 0.\n" << desc << "\n"; return 1; }

		// add all pgm files from the input directory to the vector
		fs::directory_iterator end_itr; // default construction yields past-the-end
		if (vm.count("input-dir"))
		{
			if ( fs::is_regular_file(iDir))
			{
				images.push_back(iDir);
			}
			else {
        ...
        if (verbose) {
            ...
            std::cout << "# running payload-limited sender with alpha = " << payload_rate << std::endl;
		...
		clock_t begin=clock();
		cost_model_config *config = new cost_model_config(payload_rate, verbose, wavelet, stc_constr_height, randSeed);
		
		
		std::ifstream file(payload_path);
		if (!file.is_open()) {
		...
		buffer << file.rdbuf();
		std::string payload_data = buffer.str();
		...
		for (int imageIndex=0; imageIndex<(int)images.size(); imageIndex++)
		{
			...
			mat2D<int> * stego = model->Embed(alpha_out, coding_loss_out, stc_trials_used, distortion, payload_data);
