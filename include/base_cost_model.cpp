...
mat2D<int> * base_cost_model::Embed(float &alpha_out, float &coding_loss_out, unsigned int &stc_trials_used, float &distortion, std::string payload_data)
{
	...
	else 
	{
        ...
        stego = mi_emb_stc_pls_embedding(this, config->payload, rng(), config->stc_constr_height, stc_max_trials, distortion, alpha_out, coding_loss_out, stc_trials_used, payload_data );
    }
