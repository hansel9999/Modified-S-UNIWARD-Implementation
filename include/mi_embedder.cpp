...
unsigned char* getMessageBits(std::string payload_data) {
    int n = payload_data.length();
        int len = n * 8;
    int *message = new int(len);
    for (int i = 0; i < n; ++i) {
        int c = int(payload_data[i]);
        for (int j = 0; j < 8; ++j)
            message[(8 * i) + j] = (c & (1 << (7 - j))) >> (7 - j);
    }
    unsigned char *message_chars = new unsigned char(len);
    for (int i = 0; i < len; ++i) {
        message_chars[i] = '0' + message[i];
    }
    message_chars[len] = '\n';
